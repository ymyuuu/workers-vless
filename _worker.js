...
function generateResponseText(cf, host) {
	...
	const isNotWorkersOrPagesDev = !host.endsWith('workers.dev') && !host.endsWith('pages.dev');
...
function generateLinks(host, ports, encryption, security, isHTTPS = false) {
	return ports.map(port =>
		`vless://${userID}@${main}:${port}?encryption=${encryption}&security=${security}${isHTTPS ? `&sni=${host}` : ''}&fp=random&type=ws&host=${host}&path=%2F%3D2048#CFW_${port}`
	);
}
